...
const CreateAccountSchema = z
	.object({
		...
		password: z.string().refine(password => {
			const exp = /^(?=.*\d)(?=.*[A-Z])(?=.*[a-z])(?=.*[a-zA-Z!#$%&? "])[a-zA-Z0-9!#$%&?]{8,20}$/;
...
export function CreateAccount() {
	...
	return (
		<form className="flex flex-col w-80" onSubmit={handleSubmit(onSubmit)}>
			...
			<input className={inputcss} {...register('name')} type="text" placeholder="Name" />
			{errors.name && <div className="text-red-500">{errors.name.message}</div>}
			<input className={inputcss} {...register('password')} type="password" placeholder="Password" />
			{errors.password && <div className="text-red-500">{errors.password.message}</div>}
			<input className={inputcss} {...register('confirmPassword')} type="password" placeholder="Repeat Password" />
